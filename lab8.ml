...
let receive_report (a : string) : unit = 
  let _ = WEvent.add_listener publish (fun () -> ignore (fakeNewsNetwork a)) in
  let _ = WEvent.add_listener publish (fun () -> ignore (buzzFake a)) in
  () ;;
